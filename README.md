...
```
pauta-ensaios/
├── index.html          # estrutura da aplicação (dashboard, tabela, estatísticas, músicos, importação)
├── css/styles.css       # tema (claro/escuro), layout responsivo, identidade visual
├── js/app.js             # toda a lógica: motor de datas, filtros, calendário, tabelas, músicos, import/export
├── data/dados.json        # dados de ensaios extraídos da planilha (regras recorrentes, não datas fixas)
├── data/musicos.json       # dados de músicos/candidatos extraídos da planilha do SAM
└── tools/
    ├── gerar_dados_json.py     # regenera data/dados.json a partir de uma planilha de Ensaios (.xlsx)
    └── gerar_musicos_json.py   # regenera data/musicos.json a partir de uma planilha do SAM (.xlsx)
```
...
Além do calendário de ensaios, o app tem uma aba **Músicos (SAM)** com o
cadastro de músicos e candidatos do ministério de música, gerado a partir da
planilha do SAM (aba única, 2.304 registros na versão atual).

Funcionalidades:

- Filtros em cascata: Nível, Status (derivado do Nível, ex.: um registro
  "RJM / Culto Oficial" aparece nas duas tags), Encarregado Regional,
  Secretário, Setor, Cidade, Localidade, Instrumento e Tipo
  (Teresina/Interior) — cada filtro mostra só as opções que existem dentro
  da combinação já escolhida nos demais, igual ao comportamento da aba de
  Ensaios
- Cards de estatística (total de integrantes, músicos/organistas,
  candidatos, oficializados, setores, cidades) que recalculam conforme os
  filtros
- Tabela com busca, ordenação por coluna, paginação e exportação para Excel
  e PDF
- Clique em uma linha abre um modal com todos os detalhes do integrante
- Distribuição (rankings) por Setor, Cidade, Instrumento e Encarregado
  Regional
- Importação de novas planilhas do SAM em **Importar Excel → Importar
  planilha do SAM (Músicos)**, com validação linha a linha (linhas sem
  `NOME` são ignoradas) e opção de substituir ou adicionar aos dados atuais
- Zona de risco própria para limpar apenas a base de músicos, sem afetar os
  ensaios

Assim como os ensaios, os dados de músicos importados pelo app ficam salvos
no `localStorage` (chave `pauta.musicos.v1`) e passam a ter prioridade sobre
`data/musicos.json`. Para regenerar `data/musicos.json` fora do navegador a
partir de uma planilha nova do SAM:
...
A planilha deve ter as colunas: `NOME`, `INSTRUMENTO`, `LOCALIDADE`,
`CARGO/MINISTÉRIO`, `NIVEL`, `Encarregado Regional`, `Secretário`,
`Musico ou Canditado`, `Setor`, `Musico ou organista`, `CLASSE`, `Cidade`,
`Tipo` — os mesmos nomes de coluna usados na planilha original do SAM.
...
- Seletor de ano no menu lateral, que recalcula automaticamente todas as
  datas do calendário
- Aba Músicos (SAM): cadastro de músicos/candidatos com filtros em cascata,
  cards de estatística, tabela com exportação, rankings por setor/cidade/
  instrumento/encarregado regional e importação própria de planilhas do SAM
  (ver seção "Módulo Músicos (SAM)" acima)
